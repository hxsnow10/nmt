...
class BaseModel(object):
  ...
  def __init__(self,
    ...
    print(self.iterator.source.get_shape().as_list())
  ...
  def build_graph(self, hparams, scope=None):
    ...
    with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
      ...
      print('Encoder outputs Name={}'.format(encoder_outputs.name))
      ## Decoder
      logits, sample_id, final_context_state = self._build_decoder(
          encoder_outputs, encoder_state, hparams)
      print('sample id Name={}'.format(sample_id.name))
      ## Loss
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
  ...
  def _build_decoder(self, encoder_outputs, encoder_state, hparams):
    ...
    print('source  name={}'.format(iterator.source.name))
    print('source length name={}'.format(iterator.source_sequence_length.name))
    ...
    with tf.variable_scope("decoder") as decoder_scope:
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        with tf.device(model_helper.get_device_str(device_id, 1)):
      ...
      ## Inference
        ...
        # Dynamic decoding
        outputs, final_context_state, self.final_sequence_lengths = tf.contrib.seq2seq.dynamic_decode(
            my_decoder,
            maximum_iterations=maximum_iterations,
            output_time_major=self.time_major,
            swap_memory=False,
            scope=decoder_scope)
        print('FINAL_SEQ_LENGTH={}'.format(self.final_sequence_lengths.name))
  ...
  def _compute_loss(self, logits):
    ...
    self.maxtime = max_time = self.get_max_time(target_output)
    #target_output_one_hot=tf.one_hot(target_output, self.tgt_vocab_size, dtype=tf.int32)
    #crossent= tf.nn.softmax_cross_entropy_with_logits(labels=target_output_one_hot, logits=logits)
