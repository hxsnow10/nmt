...
def run_external_eval(infer_model, infer_sess, model_dir, hparams,
  ...
  if hparams.test_prefix:
    ...
    test_scores = _external_eval(
        ...
        summary_writer,
        save_on_best=False)
  with infer_sess as sess:
      with infer_model.graph.as_default():
        saver0 = tf.train.Saver()
        saver0.save(sess, hparams.out_dir + '/inference_model')
        saver0.export_meta_graph(hparams.out_dir + '/inference_model'+'.meta')
  print('INFERNECE SAVED')
...
def train(hparams, scope=None, target_session=""):
  ...
  config_proto = utils.get_config_proto(
      log_device_placement=log_device_placement, gpu=1)
  config_proto2 = utils.get_config_proto(
      log_device_placement=log_device_placement, gpu=0)

  train_sess = tf.Session(
      target=target_session, config=config_proto, graph=train_model.graph)
  eval_sess = tf.Session(
      target=target_session, config=config_proto2, graph=eval_model.graph)
  infer_sess = tf.Session(
      target=target_session, config=config_proto2, graph=infer_model.graph)
  ...
  run_full_eval(
      ...
      sample_tgt_data)
  print('First evaluation finished ')
  ...
  while global_step < num_train_steps:
    ...
    try:
      ...
      utils.print_out('{},{}'.format(global_step,time.time()))
      continue
